...
- [ ] **2. Retrain `EvaluationWeights` for the new trading, Classic.** Weights were fitted when Expert made only single-resource offers, so they describe a policy that no longer exists. Train Expert vs Expert across all four table mixes; ship only if no mix regresses and Expert-vs-Expert improves.
- [ ] **3. Repeat 1 and 2 for the 25-point Expanded mode.** Its sweep must count an unfinished game as a loss for every seat, or the optimiser learns to stall. Open: four Expert bots still fail to finish 2 of 40 seeded Expanded games, where the shipping heuristic finishes 40.
