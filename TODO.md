...
- [ ] Measure the evaluation bot against an opponent this repository did not design. **Now the highest-value open item:** the weights are fitted against `eval`, and the 67.2%/47.3% split shows some of what was learned is specific to it. Whether they hold against a third opponent is unknown. Every strength number here is against our own heuristic, which measures how well a candidate exploits one opponent rather than how well it plays. Catanatron is GPL-3.0: usable as an external benchmark process, never linked into the app.
- [x] **Sweep `EvaluationWeights`.** Fifty SPSA iterations against frozen `eval`, validated on held-out seeds against `balanced`: **+4.7 points, McNemar p = 0.010**, lifting the bot from 43.0% to 47.3% against a 25% null (since raised to **62.4%** by the trade fix). Most of the apparent gain (67.2% against the training opponent) was exploitation of `eval` rather than strength - both columns are in [the delivery note](docs/AI_summaries/2026-09-14-position-evaluation-bot.md). `sim --weights` is the seam, and `BotWeights` can now be swept the same way.
