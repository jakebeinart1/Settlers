...
- [ ] **Sync all ghost and rating data live across devices.** Jake, 2026-09-25: ratings, ghosts (every version), game logs, decision records and per-seat stats should be synced live, not kept per phone. Today the app has **no backend and no network**, and ghost data stays on one device, apart from Jake's ghost bundled into the build (`docs/superpowers/specs/2026-09-25-ghost-opponents-design.md`).
