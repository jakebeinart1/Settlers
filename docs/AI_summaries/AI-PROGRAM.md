...
| **3. Make training faster without weakening learning** | Export and sampled compound parity passed. First [4090 profile](2026-09-06-training-profile.md) overlapped another GPU job; timing invalid. Enforce the shared lock, then spend only the remaining two-minute budget on the clean pair. | Repeated profiles identify rollout/inference/transfer/update/evaluation costs; a bounded before/after experiment improves time-to-quality without correctness or playing-strength regression. |
